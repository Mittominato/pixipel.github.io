...
const myElement = document.querySelector('.galeri');
let jumlah = 10;
let images = [];
let searchValue = "";
...
//randomImg
function randomImg() {
content.innerHTML = `<div class="col-span-2"><div class="block w-8 h-8 bg-slate-700 mx-auto rounded-full animate-bounce"></div>
    <h4 class="text-center">Memuat Gambar</h4></div>`;
...
function loadingSearch() {
    content.innerHTML = `<div class="col-span-2"><div class="block w-8 h-8 bg-slate-700 mx-auto rounded-full animate-bounce"></div>
    <h4 class="text-center">Sedang Mencari gambar <span class="font-bold italic">${searchValue}</span></h4></div>`;
    lihatlebih.classList.add("hidden");
}
//function scroll
...
async function next() {
    ...
    .finally(() => {
        lihatlebih.innerHTML = "Lihat Lainnya";
        download();
    });
};
...
function download() {
    [...document.querySelectorAll(".imageContent")].forEach(img => {
        img.addEventListener("click", () => {
            ...
            <button class="hight py-2 px-4 rounded-full bg-sky-500 mt-1 text-white text-sm font-bold mx-auto md:mt-5" type="button">Download <span class="font-semibold">- hight</span></button>
            </a>
            <a href="https://pixabay.com/id/images/download/${img.dataset.iddownload}.jpg?attachment">
            <button class="hight py-2 px-4 rounded-full bg-sky-500 mt-1 text-white text-sm font-bold mx-auto md:mt-5" type="button">Download <span class="font-semibold">- ultra hight</span></button>
            </a>
            <button class="closeDownload py-2 px-32 rounded-full bg-slate-700 mt-5 mb-5 text-white font-bold text-sm mx-auto" type="button">Close</button>
            <p class="text-[.8rem] text-slate-700 italic underline mt-10 mb-5">Buat akun <a href="pixabay.com" class="font-bold text-sm">Pixabay</a> untuk mendownload gambar di <span class="font-bold text-sm">Pixipel </span> kualitas medium hingga ultra tanpa chapcha.</p>`;
            close();
        });
    });
};
